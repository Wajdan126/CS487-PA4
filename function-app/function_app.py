...
def validate_activity(order: dict) -> dict:
    ...
    r = requests.post(validate_url, json=order, timeout=20)
    r.raise_for_status()
...
def report_activity(order: dict) -> str:
    ...
    safe_order_id = re.sub(r"[^a-zA-Z0-9-]", "-", order_id).lower()
    name = f"ci-report-{safe_order_id}-{int(time.time())}"
    ...
    try:
        ...
        for _ in range(60):
            ...
            if info.instance_view and info.instance_view.state:
                final_state = info.instance_view.state

            if info.containers and info.containers[0].instance_view:
                current_state = info.containers[0].instance_view.current_state
                if current_state and current_state.state:
                    final_state = current_state.state

            if final_state in ("Succeeded", "Terminated"):
                break

            if final_state == "Failed":
                raise Exception("Report container failed")
